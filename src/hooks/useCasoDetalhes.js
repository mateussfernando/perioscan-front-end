...
export default function useCasoDetalhes(casoId) {
  ...
  const handleCasoChange = (e) => {
    ...
    } else if (name.startsWith("victim.")) {
      const fieldPath = name.split(".")
      setCasoEditado((prev) => {
        const newVictim = { ...prev.victim }
        let current = newVictim
  ...
  const salvarCaso = async (e) => {
    ...
    try {
      ...
      if (userRole !== "admin" && userRole !== "perito" && userRole !== "assistente") {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Falha ao atualizar caso: ${response.status} - ${errorText}`)
      }
  ...
  const excluirCaso = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        throw new Error(`Falha ao excluir caso: ${response.status} - ${errorText}`)
      }
  ...
  const atualizarStatusCasoParaFinalizado = useCallback(async () => {
    try {
      ...
      console.log("Atualizando status do caso para finalizado:", dadosAtualizacao)
      ...
      if (!response.ok) {
        const errorText = await response.text()
        throw new Error(`Falha ao atualizar status do caso: ${response.status} - ${errorText}`)
      }
