...
export default function useVitima(casoId, mostrarNotificacao) {
  ...
  const salvarVitima = async (dadosVitima) => {
    ...
    try {
      ...
      // Fazer a requisição para adicionar a vítima ao caso
      const response = await fetch(`https://perioscan-back-end-fhhq.onrender.com/api/cases/${casoId}/victim`, {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        ...
        mostrarNotificacao("Vítima adicionada com sucesso!", "sucesso")
