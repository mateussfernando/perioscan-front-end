...
export default function OdontogramaModal({ aberto, onClose }) {
  ...
  const salvarComentario = () => {
    setComentarios((prev) => ({ ...prev, [denteSelecionado]: comentarioAtual }));
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ width: '100%', maxWidth: 800, margin: '0 auto' }}>
              <OdontogramaSVG style={{ width: '100%', height: 'auto', maxWidth: '100%', maxHeight: '70vh', display: 'block' }} onDenteClick={handleDenteClick} aberto={aberto} />
            </div>
          </div>
        </div>
      )}
      {modalComentario && (
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={() => setModalComentario(false)} style={{
              position: 'absolute',
              top: 10,
              right: 14,
              background: 'none',
            ...
            <h4 style={{marginTop: 0}}>Comentário do dente {denteSelecionado && denteSelecionado.replace('tooth', '')}</h4>
            <textarea
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => setModalComentario(false)} style={{ padding: '8px 16px', borderRadius: 4, border: 'none', background: '#eee', cursor: 'pointer' }}>Fechar</button>
              <button onClick={salvarComentario} style={{ padding: '8px 16px', borderRadius: 4, border: 'none', background: '#222', color: '#fff', cursor: 'pointer' }}>Salvar</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{ position: 'relative' }}>
      ...
      {modalAberto && (
        <div style={{
          ...
          <div style={{ background: '#fff', borderRadius: 8, padding: 24, minWidth: 320, maxWidth: 400, boxShadow: '0 4px 24px #0002' }} onClick={e => e.stopPropagation()}>
            <h3 style={{ marginBottom: 12 }}>Comentário do dente <b>{denteSelecionado}</b></h3>
            <textarea
              value={comentarioAtual}
              onChange={e => setComentarioAtual(e.target.value)}
              rows={5}
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
              <button onClick={() => setModalAberto(false)} style={{ padding: '6px 18px', borderRadius: 4, border: 'none', background: '#eee', cursor: 'pointer' }}>Fechar</button>
              <button onClick={salvarComentario} style={{ padding: '6px 18px', borderRadius: 4, border: 'none', background: '#222', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>Salvar</button>
            </div>
          </div>
        </div>
      )}
