    ...
    <tr className="evidencia-row">
      <td onClick={() => onVerEvidencia(evidencia)}>
        <div className="evidencia-info-cell">
          <span className="evidencia-titulo">
            {evidencia.description || `Evidência ${index + 1}`}
          </span>
          <span className="evidencia-data">
            {formatarData(evidencia.createdAt || evidencia.collectionDate)}
          </span>
        </div>
      </td>
      <td onClick={() => onVerEvidencia(evidencia)}>
        <span
          className={`evidencia-tipo ${
            evidencia.type === "image" ? "tipo-imagem" : "tipo-texto"
          }`}
        >
      ...
      <td className="acoes-cell">
        <button
          ...
          onClick={() => onVerEvidencia(evidencia)}
          title="Ver detalhes"
        >
