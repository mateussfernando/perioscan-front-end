...
import "../styles/casos-filtro.css"
import AsideNavbar from "../components/AsideNavBar"
import CasosFiltro from "../components/casos/CasosFiltro"
import { useEffect, useState } from "react"
import { useRouter } from "next/navigation"
import { Plus, X, Loader, MapPin, FileText, Eye } from "lucide-react"
...
export default function MainCasos() {
  ...
  useEffect(() => {
    filtrarCasos()
  }, [casos, termoBusca, filtrosAtivos])
  ...
  const filtrarCasos = () => {
    ...
    if (filtrosAtivos.status) {
      resultado = resultado.filter((caso) => caso.status === filtrosAtivos.status)
    }
    ...
    <div className="main-conteiner-casos">
      ...
      <div className="container-casos">
        ...
        <div className="tabela-container">
          <CasosFiltro onSearch={handleSearch} onFilter={handleFilter} />
                ...
                {loading ? (
                  ...
                  casosFiltrados.map((caso) => (
                    <tr key={caso._id}>
                      ...
                      <td>
                        <span className={`status ${getStatusClassName(caso.status)}`}>{caso.status || "--"}</span>
                      </td>
                      <td>
                        <button
          ...
          <div className="mobile-view">
            {loading ? (
              ...
              <div className="error-card">Erro ao carregar casos: {error}</div>
            ) : casosFiltrados.length === 0 ? (
              <div className="empty-card">
                {casos.length === 0 ? "Nenhum caso encontrado." : "Nenhum caso corresponde aos filtros aplicados."}
              </div>
            ) : (
              casosFiltrados.map((caso) => (
                <div key={caso._id} className="caso-card" onClick={() => verDetalhesCaso(caso._id)}>
                  ...
                  <div className="caso-card-body">
                    <div className="caso-card-info">
                      ...
                      <p>
                        <strong>Abertura:</strong> {formatarData(caso.openDate)}
                      </p>
                      <p>
                        <strong>Criado por:</strong> {caso.createdBy?.name || "--"}
                      </p>
                    </div>
                    <button
                      className="btn-ver-caso"
