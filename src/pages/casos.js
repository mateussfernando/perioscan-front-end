...
export default function MainCasos() {
  ...
  const filtrarCasos = () => {
    ...
    if (filtrosAtivos.status) {
      const statusFiltro = filtrosAtivos.status
        .toLowerCase()
        .replace(/[^a-z]/g, "");
      resultado = resultado.filter((caso) => {
        const casoStatus = (caso.status || "")
          .toLowerCase()
          .replace(/[^a-z]/g, "");
  ...
  // Função para lidar com mudanças nos campos do formulário do caso
  ...
  // Função para lidar com mudanças nos campos da vítima
  const handleVitimaChange = (vitimaData) => {
    console.log("Dados da vítima atualizados:", vitimaData);
    setNovoCaso((prev) => ({
      ...prev,
      victim: vitimaData,
    }));
  ...
  const criarCaso = async (e) => {
    ...
    try {
      ...
      // Preparar os dados do caso (sem a vítima)
      const dadosCaso = { ...novoCaso };
      ...
      if (novoCaso.type === "outro" && novoCaso.tipoPersonalizado.trim()) {
        dadosCaso.type = novoCaso.tipoPersonalizado.trim();
      }

      // Remover campos que não devem ser enviados para o caso
      delete dadosCaso.tipoPersonalizado;
      delete dadosCaso.victim; // Remover vítima dos dados do caso

      console.log(
        "Criando caso com dados:",
        JSON.stringify(dadosCaso, null, 2)
      );

      // PASSO 1: Criar o caso
      ...
      if (!responseCaso.ok) {
        const errorText = await responseCaso.text();
        throw new Error(
          `Falha ao criar caso: ${responseCaso.status} ${errorText}`
        );
      }

      const casoCriado = await responseCaso.json();
      console.log("Caso criado com sucesso:", casoCriado);

      // Obter o ID do caso criado
      const casoId =
        casoCriado.data?._id ||
        casoCriado.data?.id ||
        casoCriado._id ||
      ...
      if (!casoId) {
        throw new Error("ID do caso não encontrado na resposta");
      }

      let vitimaMessage = "";

      // PASSO 2: Criar vítima se as informações foram preenchidas
      if (adicionarVitima && novoCaso.victim) {
        const victim = novoCaso.victim;

        console.log("Dados da vítima para verificação:", victim);

        // Verificar se pelo menos alguns campos obrigatórios da vítima foram preenchidos
        const temDadosVitima =
          victim.identificationType ||
          victim.name ||
          victim.gender ||
          victim.age ||
          victim.birthDate ||
          victim.referenceCode ||
          victim.nic;

        console.log("Tem dados da vítima?", temDadosVitima);

        if (temDadosVitima) {
          console.log(
            "Criando vítima com dados:",
            JSON.stringify(victim, null, 2)
          );

          try {
            // Preparar dados da vítima
            const dadosVitima = {
              ...victim,
              caseId: casoId, // Vincular diretamente ao caso
            };

            // Remover campos vazios
            Object.keys(dadosVitima).forEach((key) => {
              if (dadosVitima[key] === "" || dadosVitima[key] == null) {
                delete dadosVitima[key];
              }
            });

            // Limpar objetos aninhados vazios
            if (
              dadosVitima.document &&
              Object.values(dadosVitima.document).every((val) => !val)
            ) {
              delete dadosVitima.document;
            }
            if (
              dadosVitima.estimatedAge &&
              Object.values(dadosVitima.estimatedAge).every((val) => !val)
            ) {
              delete dadosVitima.estimatedAge;
            }

            console.log("Dados finais da vítima para envio:", dadosVitima);

            // Validações específicas da vítima
            if (!dadosVitima.identificationType) {
              console.warn(
                "Tipo de identificação não informado, usando 'não_identificada'"
              );
              dadosVitima.identificationType = "não_identificada";
            }

            if (
              dadosVitima.identificationType === "identificada" &&
              !dadosVitima.name
            ) {
              throw new Error("Nome é obrigatório para vítimas identificadas");
            }

            if (
              dadosVitima.identificationType === "não_identificada" &&
              !dadosVitima.referenceCode
            ) {
              console.warn(
                "Código de referência não informado para vítima não identificada"
              );
            }

            // Criar a vítima
            const responseVitima = await fetch(
            ...
            const responseVitimaText = await responseVitima.text();
            console.log("Resposta da criação da vítima:", responseVitimaText);

            if (!responseVitima.ok) {
              console.error("Erro ao criar vítima:", responseVitimaText);
              vitimaMessage = " (Houve um problema ao criar a vítima)";
            } else {
              const vitimaCriada = JSON.parse(responseVitimaText);
              console.log(
                "Vítima criada e vinculada com sucesso:",
                vitimaCriada
              );
              vitimaMessage = " e vítima vinculada";
            }
          } catch (errorVitima) {
            console.error("Erro ao processar vítima:", errorVitima);
            vitimaMessage =
              " (Erro ao processar vítima: " + errorVitima.message + ")";
          }
        } else {
          console.log("Nenhum dado da vítima foi preenchido");
        }
      }
      ...
      alert("Caso criado com sucesso!" + vitimaMessage);
                            ...
                            <div className="victim-info">
                              ...
                              {caso.victim.identificationType && (
                                <div className="victim-status">
                                  <span
                                    className={`identificacao-${caso.victim.identificationType?.replace(
                                      "_",
                                      "-"
                                    )}`}
                                  >
          ...
          <div className="modal-overlay" onClick={fecharModalNovo}>
            ...
            >
              ...
              >
                ...
                >
                  <FileText
                    style={{ color: abaAtiva === "caso" ? "#fff" : "#000" }}
                    size={18}
                  />
                  Adicionar Caso
                </button>
                <button
                  ...
                  onClick={() => {
                    if (adicionarVitima) setAbaAtiva("vitima");
                  }}
                  disabled={!adicionarVitima}
                >
                  <User
                    style={{ color: abaAtiva === "vitima" ? "#fff" : "#000" }}
                    size={18}
                  />
                  Adicionar Vítima
                </button>
                      ...
                      >
                        {criandoCaso ? (
                          ...
                          <>
                            <Plus size={16} />
                            <span>
                              Criar Caso{adicionarVitima ? " + Vítima" : ""}
                            </span>
                          </>
                        )}
                      </button>
                ...
                {abaAtiva === "vitima" && adicionarVitima && (
                  <div style={{ padding: 0 }}>
                    <ModalAdicionarVitima
                      ...
                      onSalvar={() => {}} // Não usado no modo inline
                      salvando={false}
                      erro=""
                      inline={true}
                      formData={novoCaso.victim}
                      onFormChange={handleVitimaChange}
                    />
                  </div>
                )}
              </div>
