...
import { FileText, Download, Search, Filter, Trash2, RefreshCw } from "lucide-react"
...
export default function Relatorios() {
  ...
  const buscarRelatorios = async () => {
    try {
      ...
      const resposta = await fetch("https://perioscan-back-end.onrender.com/api/reports", {
        method: "GET",
        headers: {
  ...
  const exportarPDF = async (relatorioId) => {
    try {
      ...
      const resposta = await fetch(`https://perioscan-back-end.onrender.com/api/reports/${relatorioId}/pdf`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const excluirRelatorio = async (relatorioId) => {
    ...
    try {
      ...
      const resposta = await fetch(`https://perioscan-back-end.onrender.com/api/reports/${relatorioId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
      ...
      if (!resposta.ok) {
        throw new Error(`Erro ao excluir relatório: ${resposta.status}`)
      }

      // Atualizar a lista de relatórios após exclusão
      setRelatorios(relatorios.filter((relatorio) => relatorio._id !== relatorioId))
      alert("Relatório excluído com sucesso!")
    } catch (erro) {
      console.error("Erro ao excluir relatório:", erro)
      alert("Falha ao excluir o relatório. Por favor, tente novamente.")
    }
  }
  ...
  const relatoriosFiltrados = Array.isArray(relatorios)
    ? relatorios.filter((relatorio) => {
        ...
        const correspondeTexto =
          titulo.toLowerCase().includes(filtro.toLowerCase()) || conteudo.toLowerCase().includes(filtro.toLowerCase())
  ...
  const relatoriosPaginados = relatoriosOrdenados.slice(indicePrimeiroItem, indiceUltimoItem)
                  ...
                  <tbody>
                    {relatoriosPaginados.map((relatorio) => (
                      <tr key={relatorio._id || `relatorio-${Math.random()}`}>
                        <td className="relatorio-titulo">{relatorio.title || "Sem título"}</td>
                        <td>
                          {relatorio.case ? (
                            <button className="btn-ver-caso" onClick={() => navegarParaCaso(relatorio.case)}>
                              Ver caso
                            </button>
                          ) : (
                            "Caso não vinculado"
                        ...
                        <td>
                          <span className={`status-badge status-${relatorio.status || "indefinido"}`}>
                        ...
                        <td>
                          ...
                          <span className={relatorio.status === "finalizado" ? "assinado-sim" : "assinado-nao"}>
